...
Temp = 273 + 1000  		# in absolute scale
...
v_list = np.array([dv * i for i in range(120)]) 		# velocity bins
...
def animate(frame):
    ...
    if frame > f_eq:
        ...
        n_data = np.array(sum_n) / (frame - f_eq)		# calculating continuous avg of the no of particles in each bins
        ax2.bar(v_list, n_data, width=dv * 0.8, align='edge')	# plotting velocity distribution histogram
...
ani.save('outputs/Velocity_Distribution.mp4', writer=wr)	# Don't forget to change location
